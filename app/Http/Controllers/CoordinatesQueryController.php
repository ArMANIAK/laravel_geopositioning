...
class CoordinatesQueryController extends Controller
{
    ...
    public function getReverseGeocoding(Request $request)
    {
        ...
        $requestedCoords = CoordinatesQuery::firstOrNew([
            'longitude' => $longitude,
            'latitude' => $latitude
        ]);
        if ($requestedCoords->response_id)
        {
            $responseAddress = Response::find($requestedCoords->response_id);
            echo "$responseAddress->address<br/>Country: {$responseAddress->country}, State: {$responseAddress->region}, City: {$responseAddress->city}<br/>";
        } else {
